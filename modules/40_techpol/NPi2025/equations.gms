...
q40_RenShare(t,regi,ShareTargetType)$(p40_RenShare(t,regi,ShareTargetType))..
  sum(TargetType2InOutEnty(ShareTargetType,enty,enty2),
...
*** Renewable SE production of output SE carrier (enty2) as second product
  ...
  =g=
  p40_RenShare(t,regi,ShareTargetType) 
  * sum(TargetType2InOutEnty(ShareTargetType,enty,enty2),
*** Total SE production of SE output SE carrier (enty2) as main product
      sum(en2en(enty3,enty2,te),
        vm_prodSe(t,regi,enty3,enty2,te)
      )
      +
*** Total SE production of SE output SE carrier (enty2) as second product
      sum(pc2te(enty4,enty3,te,enty2),
        max(0, pm_prodCouple(regi,enty4,enty3,te,enty2)) 
        * vm_prodSe(t,regi,enty4,enty3,te)
      )
    )
  ;
