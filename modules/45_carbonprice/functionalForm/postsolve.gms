...
***------------------------------------------------------------------------------------------------------------------------------------------------
if((cm_iterative_target_adj eq 4) OR (cm_iterative_target_adj eq 44),
...
*** 3. add a funnel to avoid excessive adjustments. 
  ...
  !! copy the information before further funnelling 
  ...
  loop(regi, 
    ...
    if ((   (pm_factorRescale_taxCO2Regi_Funneled2(iteration, regi) le p45_FunnelLower(iteration))  !! le because last iteration may have already been adjusted upwards
        AND (pm_factorRescale_taxCO2Regi_Funneled2(iteration-1, regi) le p45_FunnelLower(iteration-1))
        AND (pm_factorRescale_taxCO2Regi_Funneled2(iteration-2, regi) le p45_FunnelLower(iteration-2))),
        pm_factorRescale_taxCO2Regi_Funneled2(iteration, regi) = max( p45_FunnelLower(iteration) / 1.2, 
                                                                      p45_factorRescale_taxCO2Regi(iteration,regi));
        );
...
!! if not alternating adjustment or lower iteration value
else
  p45_factorRescale_taxCO2Regi_Final(iteration, regi) = pm_factorRescale_taxCO2Regi_Funneled2(iteration, regi);
) ;
...
if((iteration.val eq 1), !! the global anchor trajectory is chosen as starting point after iteration 1 -> all regions have the same shape
      p45_taxCO2eq_anchorRegi(ttot, regi)$(ttot.val gt 2005) = p45_taxCO2eq_anchor(ttot);
    else                                             !! For all iterations > 1: get the CO2 tax that was used in this iteration, to be adjusted
...
if(cm_CPslopeAdjustment = 0, 
    ...
    if(cm_iterative_target_adj = 44 , !! After cm_peakBudgYr, the global anchor trajectory increases linearly with fixed annual increase given by cm_taxCO2_IncAfterPeakBudgYr
...
!! Option 4B: slope of CP is adjusted
if(cm_CPslopeAdjustment = 1, 
    p45_taxCO2eq_anchorRegi(ttot,regi)$(ttot.val ge 2005) =  0; !! reset carbon price trajectory to track errors
    ...
    !! B2.3b: Set the rescaled anchor trajectory as of cm_peakBudgYr
      ...
      p45_taxCO2eq_anchorRegi(ttot,regi)$(ttot.val ge cm_startYear AND ttot.val lt 2100) = 
                                          sum(ttot3$(ttot3.val eq s45_YearBeforeStartYear), p45_taxCO2eq_anchorRegi(ttot3,regi)) !! CO2 tax in last fixed period
                                                  + (ttot.val - s45_YearBeforeStartYear) * p45_CarbonPriceSlope(regi) ;
    ); !! whether it is increase until 2100 or PB shape
  );!! CP slope adjustment 
