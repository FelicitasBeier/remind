...
if( (ord(iteration) le 25 and ord(iteration) ge cm_startIter_EDGET and (mod(ord(iteration), 2) eq 0))
    ...
    *** Track runtime
    putclose runtime gyear(now):0:0 "-" gmonth(now):0:0 "-" gday(now):0:0 " " ghour(now):0:0 ":" gminute(now):0:0 ":" gsecond(now):0:0 ",iterativeEdgeTransport,start" iteration.val:0;

    Execute "Rscript -e 'edgeTransport::iterativeEdgeTransport()'";

    *** Track runtime
    putclose runtime gyear(now):0:0 "-" gmonth(now):0:0 "-" gday(now):0:0 " " ghour(now):0:0 ":" gminute(now):0:0 ":" gsecond(now):0:0 ",iterativeEdgeTransport,end" iteration.val:0;
    ...
    !! load FE-to-ES results from EDGE-Transport into auxiliary parameter
